...
## 架构方向升级（2026-06-05）：个人私有化 AI 记忆层

> 本节为**最新方向**，优先级高于下文 MVP 阶段的部分历史约定（已在相应处标注 superseded）。

CloudMind 的目标从"自称记忆层的 RAG"正式升级为 **大而完备、个人私有化部署的 AI 记忆层**，能力对标 **mem0 / Zep-Graphiti / Letta(MemGPT) / Cognee**，差异化主打 **BYOC 自托管 + 数据主权**（全栈在用户自己的 Cloudflare 账户内，无外部记忆 SaaS、无托管图数据库）。
...
- 先做闭环，再做复杂度
- 先做单用户，再考虑多人/协作
- ~~先做可用采集，再做复杂知识图谱~~（**已升级**：见「架构方向升级」，L2 改为完整知识图谱）
- 先做全文与语义检索，再做高级自动推理
...
- 复杂多人权限系统
- 高级图谱可视化
- ~~自动实体对齐/复杂知识合并~~（**已纳入范围**：见「架构方向升级」ADR-001，L2 完整知识图谱的核心）
- 重型工作流编排界面
- 视频全文转录
- 图片 OCR 全流程
- 社交化功能
