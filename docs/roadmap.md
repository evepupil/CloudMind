...
知识图谱、记忆生命周期、Web 管理和数据主权已经形成闭环，当前主线是完成
可重复、可验证的稳定发布。
...
| [M7](#m7-工程质量与发布) | 把本地门禁扩展为开发前置保障和可重复发布 | 进行中 | 贯穿已交付里程碑 | [工程质量与发布](模块设计/工程质量与发布.md) | 前置 CI 与 Cloudflare 集成门禁持续通过，并完成一次带迁移、回滚和生产冒烟的可复现发布 |
