...
| [M6](#m6-数据主权与生命周期) | 兑现不可变原始快照、完整导出和可重建的数据主权 | 未开始 | 阶段 A：M0；阶段 B：M3 | [数据主权与生命周期](模块设计/数据主权与生命周期.md) | 文本、MCP 与 Agent 输入先有不可变快照；随后整库数据和 R2 文件可校验导出、导入与重建，遗忘后的跨存储状态一致 |
...
Vectorize 保存派生索引。URL 与 PDF 已保留 R2 原始内容；文本输入目前只保存在
D1，处理完成后正文可能收敛为处理预览，三类来源的完整不可变原文由 M6 负责。
...
本里程碑分两个阶段。阶段 A 在 M3 前完成文本、MCP 和 Agent 输入的不可变原始
快照，保证后续自动捕获不会只留下可变预览。阶段 B 在 M3 后完成版本化、分页或
流式的整库导出，覆盖 D1 元数据、R2 原文与派生文件、校验清单、导入、向量重建
和恢复演练。`forget` 的产品入口属于 M3，跨 D1/R2/Vectorize 的清理与恢复规则
由本里程碑约束。
...
5. M7 发布收尾：建立版本、迁移、部署、回滚和生产验收流程。
