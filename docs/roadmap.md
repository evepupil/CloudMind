...
Vectorize 恢复和 fresh-resource 真实演练已完成。`forget` 的产品入口属于 M3；当前
剩余工作是硬删除审计，以及跨 D1/R2/Vectorize 的完整清理和一致性验收。
...
1. M6 阶段 B：完成硬删除审计、跨存储清理和一致性验收。
2. M5：按 eval 样本推进相对时间、评价、聚合、强化和关联能力。
3. M7 发布收尾：建立版本、迁移、部署、回滚和生产验收流程。
