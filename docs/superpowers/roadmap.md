...
| M2 | 拆分大文件 | d1-asset-repository、chat/service、indexing-policy 按职责拆分 | `planned` | P0 | M1 | — |
| M3 | 类型安全加固 | 工作流状态解析加 Zod 校验，补 Logger 核心接口 | `planned` | P1 | M2 | — |
...
1. **启动 M2**：按子接口拆分 `d1-asset-repository.ts`（1413行 → 4 模块）
2. **拆分 `chat/server/service.ts`**（1296行 → 3 模块）
3. **拆分 `indexing-policy.ts`**（982行 → 3 模块）
...
- **M1 完成**（2026-05-31）：commit `0f6e062`
  - 新建 `shared-workflow-steps.ts`（~597行），暴露 11 个步骤工厂函数 + `buildSharedIngestSteps` 组合函数
  - 3 个 workflow 文件从 ~1660 行减至 ~470 行（净减 1230 行）
  - 修复 5 处 `catch {}`：runtime.ts（×2）、ingest/service.ts、note-ingest-workflow.ts（已迁入共享步骤）、asset-workflows-page.tsx
  - 修复 Biome lint 警告：`search/server/service.ts:272` useOptionalChain
  - 验证：typecheck 零错误、lint 零警告、18/20 测试文件通过（2 个预存失败：MCP auth + workers-ai provider）
