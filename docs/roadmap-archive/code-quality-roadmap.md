...
| M3 | 类型安全加固 | 工作流状态解析加 Zod 校验，补 Logger 核心接口 | `done` | P1 | M2 | M3.1 + M3.2 已完成：共享 workflow steps 关键 state 读取改为 Zod 校验；Logger 类型端口移入 core，feature 层不再直接依赖 platform logger |
| M4 | 补测试覆盖 | indexing-policy、auto-enrichment、runtime 加单元测试 | `active` | P1 | M2 | M4.1 已完成：新增 indexing-policy characterization tests，覆盖 descriptor/access/facet/assertion 关键启发式 |

---

## 活跃工作

- M4：补测试覆盖。M4.1 `indexing-policy.ts` 单元测试已完成，M4.2 `auto-enrichment.ts` 单元测试待做。

---

## 下一步建议

1. **启动 M4.2**：补 `auto-enrichment.ts` 单元测试
2. **继续 M4.3**：补 `runtime.ts` 工作流运行时单元测试
3. **补充 Workers AI embedding 测试**：覆盖 `WorkersAIProvider.createEmbeddings`
...
**问题**：`createLogger` 在 `src/platform/observability/logger.ts`，多个 feature 文件直接 import 平台实现。

**方案**：
- 新增 `src/core/logging/ports.ts`，定义 `Logger` 接口（info/warn/error 方法）
- `createLogger` 实现该接口
- Feature 层通过 `@/core/logging/logger` 入口获取日志能力，避免直接 import platform logger

**进展**：
- 已新增 `src/core/logging/ports.ts` 和 `src/core/logging/logger.ts`
- `src/platform/observability/logger.ts` 已实现 core `Logger` 端口
- 已将 chat / ingest / mcp / search / workflows feature 层 logger import 收敛到 core logging 入口
...
| 文件 | 行数 | 测试策略 |
|------|------|----------|
| `indexing-policy.ts` | 982 | `done`：新增 6 个 characterization tests，覆盖 descriptor/access/facet/assertion 关键启发式 |
...
- **M4.1 完成**（2026-05-31）：新增 `tests/unit/features/workflows/server/indexing-policy.test.ts`，覆盖 descriptor 推导、source metadata fallback、public/private/restricted access policy、facet 合并清洗去重、assertion 摘要/决策/约束提取；`pnpm typecheck`、`pnpm lint`、`pnpm test` 全部通过
- **M3.2 完成**（2026-05-31）：新增 core Logger 端口与 core logging 入口，platform logger 改为实现 core 类型，feature 层 logger import 收敛到 `@/core/logging/logger`；`pnpm typecheck`、`pnpm lint`、`pnpm test` 全部通过
