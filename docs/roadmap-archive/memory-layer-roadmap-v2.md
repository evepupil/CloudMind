...
> 归档于 2026-07-22。本文只保存历史计划，当前状态以
> [`docs/roadmap.md`](../roadmap.md) 为准。

> 状态：**v2 重排 · 2026-06-07**（定位定盘，详见 [`docs/vision.md`](../vision.md)）
> 转向：从「引擎优先 / 图谱中心 / 对标 mem0」重排为「**价值优先 / 个人化 recall 先行 / 按检索模态补全**」。地基（P1 检索）保留；顺序与重心改变。

## v2 重排：按「检索模态」组织，个人化优先

**组织原则**：按**检索模态**做能力（通用、跨任何内容），**不**按「日记/习惯/荣誉/情绪」这类具体场景做（内容只是喂进来的数据，无通用性）。模态表见 [`docs/vision.md`](../vision.md)（M1 语义 / M2 个人 / M3 时间 / M4 联想 / M5 评价超级值 / M6 聚合）。

### Phase A — 心脏：个人化 recall（M2）+ 护地基　← 现在做
- **A0** 修 L1 不可变 bug：URL 类资产原始快照被 reprocess 覆盖（确定性 R2 key + 每次 reprocess 重抓），违「50 年胶囊」铁律 — `S`
- **A1** 个人记忆条目模型：`kind`(偏好/规范/属性) + `domain/主题标签` + `可见性`；大半复用现有 note + domain + tag + aiVisibility，**不依赖实体/图谱** — `M`
- **A2** `remember`(写) + `recall`(读：吃子查询 → 语义+主题+可见性过滤 → 整捆返回) 两个 MCP 动词；工具描述引导调用方 AI 自行发散子查询/关键词 — `M`
- **A3** 可见性门控：条目敏感级 + token 信任级（先「普通/私密」两档） — `M`
- **验收**：用「我该买房吗」「帮我做个网站」在生产端到端跑通——AI 发散子查询 → 取你的背景/偏好 → 懂你地回答。

### Phase B — 补全检索模态（护城河）
- **B1** 时间检索（M3）：日期范围 / 相对事件锚定 / 情节召回 — `M`
- **B2** 评价/超级值（M5）：入库时给条目打**评价维度**标签（重要性/情感效价/成就感…，通用、跨内容，**非日记专用**）+ 超级值排序；novel 维度 map-reduce 逐条打分兜底 — `L`
- **B3** 聚合/算一遍（M6）：对一批做摘要/计数/趋势（「去年几件大事」「共几次」）——真·非 RAG 超能力 — `L`
- **B4** 联想（M4）精修：确保图检索把关系类查询服务好（多数已集成，按需调） — `M`

### Phase C — 兑现主权 + 收尾
- **C1** 导出（整库 JSON/NDJSON + R2 原文，可脱离 CF 重建）——兑现「永久>平台」，**正式灌库前必做** — `M`
- **C2** `forget` 动词（用户显式，软删为主，hard 可选） — `S`
- **C3** 图谱整合/社区（仅服务关系类查询，按需，不再优先） — `L`

### 明确决定（砍 / 降级 / 推迟）
- **砍**：自动按年龄/低频遗忘（违 vision 原则 3）。原 P3「sleep-time 遗忘」取消；sleep-time 仅保留**一致性修复**（已落地 T5-i1：漂移边失效 + 重复去重，均软标记）。
- **降级**：L2 图谱 consolidation / community（原 P3「整合/社区」、原计划 T5-i3/i4）—— 锦上添花，非体验驱动。
- **推迟（不遗忘）**：导出 → Phase C，但任何 real-data 里程碑前就位。

> **进度锚点**：P1 检索地基 ✅ / P2 分层 ✅ / P3 心脏 T1–T4 ✅（智能写调和·图检索·显著性·访问写回）+ T5-i1 ✅（scheduled + 图谱修复）。下一刀 ＝ **Phase A0 + A1/A2**。

---

> 以下为 **v1 原始结构与任务清单（历史 + 明细存档）**：P1 承重墙已完成；P2/P3 的 schema 与图谱任务明细保留作实现参考；P4「记忆面」部分被 Phase A（个人化 recall）提前取代/收编。**方向以上方 v2 与 [`docs/vision.md`](../vision.md) 为准。**
