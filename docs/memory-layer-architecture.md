...
|---|---|---|---|---|
| lexical（仅词面） | 1.0000 | 0.9600 | 0.9730 | 0.9633 |
| dense（仅向量） | 1.0000 | 0.9600 | 0.9705 | 0.9600 |
| fused（跨通道融合·T6） | 1.0000 | 0.9800 | 0.9820 | 0.9733 |
| **reranked（融合+重排+MMR·T7）** | **1.0000** | **0.9800** | **0.9852** | **0.9800** |

- **fused > 单通道**、**reranked > fused**：融合与重排都带来可度量增益（验证 T6/T7）。
- 棘轮锁在 reranked 实测值（MRR≥0.98 / nDCG@10≥0.985 / MAP≥0.98），已高于 P1-T0 旧基线（0.9750/0.9815/0.9750）；并额外校验 rerank 不劣化 fused、过滤正确性零违例。
